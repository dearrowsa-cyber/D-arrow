  ...
  try {
    ...
    if (existingPlans > 0) {
      return NextResponse.json({
        success: true,
        message: 'Pricing plans already initialized',
        count: existingPlans,
      });
    }

    // Seed database with default pricing data
    for (let i = 0; i < defaultPricingData.length; i++) {
      const plan = defaultPricingData[i];
      await prisma.pricingPlan.create({
        data: {
          ...
          priceUnitAr: plan.priceUnitAr,
          priceUnitEn: plan.priceUnitEn,
          noteAr: plan.noteAr,
          noteEn: plan.noteEn,
    ...
    return NextResponse.json({
      success: true,
      message: 'Pricing plans initialized successfully',
      count: defaultPricingData.length,
    });
  } catch (error) {
    console.error('Error initializing pricing plans:', error);
    return NextResponse.json({
      success: false,
      error: 'Failed to initialize pricing plans',
    }, { status: 500 });
  }
}
