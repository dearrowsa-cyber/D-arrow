...
export default function KeywordDetail() {
  ...
  if (loading || !data)
    return <div className="admin-content">جاري تحميل بيانات الكلمة...</div>;

  const latest = data.rankings?.[0] || {
    position: 0,
    clicks: 0,
    impressions: 0,
    ctr: 0,
      ...
      >
        ...
        <div style={{ flex: 1 }}>
          <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
            ...
            {data.group && (
              <span className="admin-badge admin-badge-info">{data.group}</span>
            )}
          </div>
          <p style={{ color: "#9CA3AF", margin: "4px 0 0" }}>
            تحليل أداء الكلمة المفتاحية عبر محرك بحث جوجل
          </p>
        </div>
      </div>
      ...
      <div className="admin-grid-2" style={{ marginBottom: "24px" }}>
        <div className="admin-card">
          <h3 style={{ marginTop: 0, marginBottom: "24px" }}>
            تاريخ الترتيب (Position History)
          </h3>
          {chartData.length > 0 ? (
        ...
        <div className="admin-card">
          ...
          {latestPageRankings.size > 0 ? (
            <table className="admin-table">
              ...
              <tbody>
                {Array.from(latestPageRankings.values()).map(
                  (pk: PageKeywordData) => (
                    <tr key={pk.id}>
                      <td
                        style={{
