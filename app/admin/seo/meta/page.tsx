  ...
  const getPageName = (slug: string) => {
    ...
    if (slug.startsWith("/services/")) {
      ...
      const serviceMap: Record<string, string> = {
        dm_smm: "إدارة السوشيال ميديا",
        dm_marketing: "التسويق الرقمي",
        dm_visual: "الإنتاج البصري",
        dm_influencer: "التسويق عبر المؤثرين",
        dm_content: "صناعة المحتوى",
        dm_exhibitions: "تنظيم المعارض",
        dm_advertising: "الحملات الإعلانية",
        dm_consultation: "الاستشارات التسويقية",
        dm_seo: "تحسين محركات البحث SEO",
        id_apps: "تطبيقات الهواتف",
        id_website: "برمجة المواقع",
        id_branding: "الهوية البصرية",
        id_software: "البرمجيات الخاصة",
        id_cloud: "الخدمات السحابية",
        re_appraisal: "التقييم العقاري",
        re_marketing: "التسويق العقاري",
        re_management: "إدارة الأملاك",
        re_photography: "التصوير العقاري",
        re_campaign: "الحملات العقارية",
        re_project_images: "تصوير المشاريع",
        re_current_eval: "تقييم الوضع الحالي",
        re_project_naming: "تسمية المشاريع",
      };
    ...
    if (slug.startsWith("/store/"))
      return `منتج: ${slug.replace("/store/", "")}`;
      ...
      {deleteId && (
        <div className="admin-modal-backdrop">
          <div className="admin-modal">
            <h3 className="admin-text-primary" style={{ margin: "0 0 12px" }}>
              هل أنت متأكد؟
            </h3>
            <p
              className="admin-text-secondary"
              style={{ margin: "0 0 24px", fontSize: 14 }}
            >
              سيتم حذف سجل بيانات السيو هذا نهائياً. لا يمكن التراجع عن هذا
              الإجراء.
            </p>
            <div
              ...
              return (
                <React.Fragment key={item.id}>
                  ...
                  >
                    ...
                    <td className="admin-text-info" style={{ fontWeight: 500 }}>
                      {getPageName(item.slug)}
                    </td>
                    <td
                      style={{
                        maxWidth: "200px",
                    ...
                    >
                      {item.title || (
                        <span className="admin-text-danger">— فارغ</span>
                      )}
                    </td>
                    <td
                      className="admin-text-secondary"
                    ...
                    <td>
                      {score !== null ? (
                        <span
                          className={`admin-badge ${score >= 80 ? "admin-badge-success" : score >= 50 ? "admin-badge-warning" : "admin-badge-danger"}`}
                        >
                          {score}/100
                        </span>
                      ) : (
                    ...
                    <tr>
                      ...
                      >
                        <div
                          style={{
                            padding: "24px",
                            borderTop: "2px solid rgba(255, 77, 109, 0.2)",
                            borderBottom: "2px solid rgba(255, 77, 109, 0.2)",
                          }}
                        >
                          {/* Header */}
                          <div
                          ...
                          >
                            <h4
                              className="admin-text-brand"
                              style={{ margin: 0 }}
                            >
                              تعديل SEO لـ: {item.slug}
                            </h4>
                          ...
                          >
                            <div>
                              <label className="admin-label">
                                عنوان السيو بالعربي (Title AR) (
                                {editData.title?.length || 0}/60)
                              </label>
                              <input
                              ...
                              >
                                <div
                                  style={{
                                    height: "100%",
                                    width: `${Math.min(100, ((editData.title?.length || 0) / 60) * 100)}%`,
                                    background:
                                      (editData.title?.length || 0) > 60
                                        ? "#EF4444"
                                        : (editData.title?.length || 0) > 40
                                          ? "#22C55E"
                                          : "#F59E0B",
                                    borderRadius: "2px",
                                    transition: "all 0.3s",
                            ...
                            <div>
                              <label className="admin-label">
                                عنوان السيو بالإنجليزي (Title EN) (
                                {editData.titleEn?.length || 0}/60)
                              </label>
                              <input
                                className="admin-input"
                              ...
                              >
                                <div
                                  style={{
                                    height: "100%",
                                    width: `${Math.min(100, ((editData.titleEn?.length || 0) / 60) * 100)}%`,
                                    background:
                                      (editData.titleEn?.length || 0) > 60
                                        ? "#EF4444"
                                        : (editData.titleEn?.length || 0) > 40
                                          ? "#22C55E"
                                          : "#F59E0B",
                                    borderRadius: "2px",
                                    transition: "all 0.3s",
                          ...
                          >
                            <div>
                              <label className="admin-label">
                                ...
                                {editData.description?.length || 0}/160)
                              </label>
                              <textarea
                              ...
                              >
                                <div
                                  style={{
                                    height: "100%",
                                    width: `${Math.min(100, ((editData.description?.length || 0) / 160) * 100)}%`,
                                    background:
                                      (editData.description?.length || 0) > 160
                                        ? "#EF4444"
                            ...
                            <div>
                              <label className="admin-label">
                                الوصف بالإنجليزي (Description EN) (
                                {editData.descriptionEn?.length || 0}/160)
                              </label>
                              <textarea
                                className="admin-textarea"
                              ...
                              >
                                <div
                                  style={{
                                    height: "100%",
                                    width: `${Math.min(100, ((editData.descriptionEn?.length || 0) / 160) * 100)}%`,
                                    background:
                                      (editData.descriptionEn?.length || 0) >
                                      160
                                        ? "#EF4444"
                                        : (editData.descriptionEn?.length ||
                                              0) > 120
                                          ? "#22C55E"
                                          : "#F59E0B",
                                    borderRadius: "2px",
              ...
              <tr>
                ...
                >
                  <p style={{ margin: "0 0 8px" }}>
                    لا توجد سجلات SEO. اضغط &quot;مزامنة الصفحات&quot; لتحميل
                    بيانات السيو لجميع صفحات الموقع.
                  </p>
                </td>
              </tr>
            )}
