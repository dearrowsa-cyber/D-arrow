...
function StoreMainContent() {
  ...
  const HERO_SLIDES = [
    {
      ...
      coupon: 'KSA50',
      ctaText: 'تصفح العروض',
      image: products[0]?.image || 'https://images.unsplash.com/photo-1505740420928-5e560c06d30e?auto=format&fit=crop&w=1200&q=85',
    ...
    },
    {
      ...
      image: products[1]?.image || 'https://images.unsplash.com/photo-1523293182086-7651a899d37f?auto=format&fit=crop&w=1200&q=85',
  ...
  return (
    ...
    >
      ...
      <div 
        className="border-b border-[#382E0E] py-2 sm:py-3 px-3 sm:px-8 transition-colors duration-300 bg-[#161205]/95 backdrop-blur-md"
      >
        <div className="max-w-[1550px] w-full mx-auto flex items-center gap-2 sm:gap-3 overflow-x-auto no-scrollbar">
          <span className="font-bold text-white text-[11px] whitespace-nowrap flex-shrink-0 hidden sm:block">نشاط المتجر:</span>
      ...
      <main className="flex-1 w-full max-w-[1550px] mx-auto px-3 sm:px-8 lg:px-12 py-4 sm:py-8 space-y-6 sm:space-y-12 lg:space-y-14">
        ...
        >
          <div className="relative min-h-[280px] sm:min-h-[400px] lg:min-h-[460px] flex items-center p-6 sm:p-12 lg:p-16">
            ...
            <div className="absolute inset-0 z-0">
              ...
              />
              <div 
                className="absolute inset-0 bg-gradient-to-t md:bg-gradient-to-r from-[#0D1117]/95 via-[#0D1117]/85 to-transparent" 
              />
            </div>
            ...
            <div className="relative z-10 max-w-2xl space-y-4 text-right">
              <div 
                className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border text-xs font-black backdrop-blur-md shadow-lg bg-[#FFE600]/20 border-[#FFE600] text-[#FFE600]"
              >
                <Sparkles className="w-4 h-4 animate-pulse text-[#FFE600]" />
                <span>{HERO_SLIDES[activeSlide % HERO_SLIDES.length].badge}</span>
              </div>
              ...
              {HERO_SLIDES.map((_, idx) => (
                <button
                  ...
                  className={`h-2.5 rounded-full transition-all cursor-pointer ${
                    activeSlide === idx ? 'w-8 bg-[#FFE600]' : 'w-2.5 bg-[#1C1707]/30 hover:bg-[#1C1707]/60'
                  }`}
                />
              ))}
            </div>
          ...
          >
            <div className="flex flex-wrap items-center justify-between gap-4 border-b border-[#382E0E] pb-5">
              <div className="flex items-center gap-3">
                <div 
                  className="w-12 h-12 rounded-2xl flex items-center justify-center shadow-lg bg-[#2D250B] border border-[#382E0E] text-[#FFE600]"
                >
                ...
                <div>
                  <h3 className="font-black text-lg sm:text-xl text-white flex items-center gap-2">
                    ...
                    <span 
                      className="px-2.5 py-0.5 rounded-full text-white text-[10px] font-bold btn-phosphor"
                    >
                      خصومات حتى 50%
                    </span>
                  </h3>
                  <p className="text-xs text-[#9CA3AF] font-bold">كميات محدودة بأسعار خاصة جداً تنتهي قريباً</p>
                </div>
              </div>

              {/* Countdown Clocks */}
              <div className="flex items-center gap-2 text-xs font-mono font-bold">
                <span className="text-[#E2E8F0] font-sans ml-1 font-black">ينتهي العرض خلال:</span>
                <div className="px-3 py-2 rounded-xl bg-[#1C1707] border border-[#382E0E] text-center min-w-[45px]">
                  <span className="text-base text-white font-black block">{String(flashTimeLeft.hours).padStart(2, '0')}</span>
                  <span className="text-[9px] text-[#9CA3AF] font-sans">ساعة</span>
                </div>
                <span className="text-[#FFE600] text-lg font-black">:</span>
                <div className="px-3 py-2 rounded-xl bg-[#1C1707] border border-[#382E0E] text-center min-w-[45px]">
                  <span className="text-base text-white font-black block">{String(flashTimeLeft.minutes).padStart(2, '0')}</span>
                  <span className="text-[9px] text-[#9CA3AF] font-sans">دقيقة</span>
                </div>
                <span className="text-[#FFE600] text-lg font-black">:</span>
                <div 
                  className="px-3 py-2 rounded-xl border-2 border-[#FFE600] text-center min-w-[45px] bg-[#2D250B] text-white shadow-sm shadow-[#FFE600]/20"
                >
                  <span className="text-base font-black block text-[#FFE600]">{String(flashTimeLeft.seconds).padStart(2, '0')}</span>
                  <span className="text-[9px] text-[#9CA3AF] font-sans font-bold">ثانية</span>
                </div>
              </div>
            </div>
                  ...
                  >
                    <div className="space-y-3">
                      ...
                      <div className="relative aspect-square rounded-2xl overflow-hidden bg-[#1C1707] border border-[#382E0E]">
                        <Link href={`/demo/store/${prod.id}`} className="block w-full h-full">
                          <img 
                            src={prod.image} 
                            alt={prod.name} 
                        ...
                        {prod.badge && (
                          <span 
                            className="absolute top-3 right-3 px-2.5 py-1 rounded-xl text-white text-[11px] font-black shadow-md btn-phosphor pointer-events-none"
                          >
                            {prod.badge}
                          </span>
                        )}
                      ...
                      {/* Info */}
                      <div>
                        <span className="text-[11px] text-[#FFE600] font-black">{prod.category}</span>
                    ...
                    <div className="pt-4 border-t border-[#382E0E] mt-4 flex items-center justify-between gap-2">
                      <div>
                        <div className="flex items-baseline gap-1.5">
                          ...
                          <span className="text-xs text-[#9CA3AF] font-bold">{settings.currency}</span>
                        </div>
                        {prod.oldPrice && (
                          <div className="flex items-center gap-1.5 text-[10px] text-slate-400 line-through">
                            <span>{prod.oldPrice} {settings.currency}</span>
                            <span className="text-[#FFE600] font-black not-line-through">وفر {savings} {settings.currency}</span>
                          </div>
                        )}
                      </div>
        ...
        <section id="catalog" className="space-y-6">
          <div className="flex flex-wrap items-center justify-between gap-4 border-b border-[#382E0E] pb-4">
            <div>
              <h3 className="font-black text-xl sm:text-2xl text-white">كتالوج المنتجات الحصرية</h3>
              <p className="text-xs text-[#9CA3AF] font-bold mt-1">تصفح كافة منتجات المتجر مع خيارات التصفية السريعة</p>
            </div>
          ...
          {filteredProducts.length === 0 ? (
            <div 
              className="p-12 text-center rounded-3xl border border-[#382E0E] space-y-3 bg-[#1C1707] shadow-sm"
            >
              <Search className="w-10 h-10 text-[#FFE600] mx-auto" />
              <p className="font-black text-base text-white">لم يتم العثور على منتجات مطابقة للبحث</p>
              <button 
                  ...
                  >
                    <div className="space-y-3">
                      ...
                      <div className="relative aspect-square rounded-2xl overflow-hidden bg-[#1C1707] border border-[#382E0E]">
                        ...
                        {prod.badge && (
                          <span 
                            className="absolute top-3 right-3 px-2.5 py-1 rounded-xl text-white text-[10px] font-black shadow-md btn-phosphor pointer-events-none"
                          >
                            {prod.badge}
                          </span>
                        )}
                      ...
                      <div>
                        <div className="flex items-center justify-between text-[11px] text-[#9CA3AF] font-bold">
                          <span className="text-[#FFE600] font-black">{prod.category}</span>
      ...
      {quickViewProduct && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in">
          <div 
            className="w-full max-w-2xl rounded-3xl border-2 border-[#FFE600] p-6 sm:p-8 space-y-6 shadow-2xl relative bg-[#1C1707] text-white"
          >
            <button 
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 items-center">
              ...
              <div className="space-y-4">
                <span className="text-xs text-[#FFE600] font-black">{quickViewProduct.category}</span>
                <h3 className="font-black text-lg sm:text-xl text-white leading-snug">{quickViewProduct.name}</h3>
                <p className="text-xs text-[#9CA3AF] leading-relaxed font-medium">{quickViewProduct.description}</p>
                
                <div className="flex items-baseline gap-2 pt-2">
                  <span className="text-2xl font-black font-mono text-[#FFE600]">
                    {quickViewProduct.price}
                  </span>
                  <span className="text-sm text-[#9CA3AF] font-bold">{settings.currency}</span>
                ...
                <div className="flex items-center gap-3 pt-4">
                  <div className="flex items-center gap-2 bg-[#1C1707] p-1.5 rounded-xl border border-[#382E0E]">
                    ...
                    <span className="w-8 text-center font-mono font-bold text-sm text-white">{quickViewQty}</span>
                    <button 
