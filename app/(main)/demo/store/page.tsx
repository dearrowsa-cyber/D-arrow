      ...
      <div dir="rtl" style={{ background: '#0B0D1F', color: '#E6E6EA', minHeight: '100vh', fontFamily: 'system-ui, sans-serif' }}>
        ...
        <header style={{ borderBottom: '1px solid rgba(255,255,255,0.06)', background: 'rgba(20,22,46,0.85)', backdropFilter: 'blur(14px)', position: 'sticky', top: 0, zIndex: 40, padding: '12px 24px' }}>
          <div style={{ maxWidth: 1200, margin: '0 auto', display: 'flex', alignItems: 'center', gap: 16 }}>
            <button onClick={() => setDetailProduct(null)} style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 10, padding: '8px 16px', color: '#E6E6EA', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, fontWeight: 600, fontSize: 13 }}>
              ← العودة للمتجر
            </button>
            <div style={{ flex: 1 }} />
            <button onClick={() => setIsCartOpen(true)} style={{ position: 'relative', background: 'linear-gradient(135deg,#FF4D6D,#FF9A3C)', border: 'none', color: 'white', padding: '8px 16px', borderRadius: 10, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, fontWeight: 700, fontSize: 13 }}>
              <ShoppingCart size={16} /> السلة
              {totalItems > 0 && <span style={{ background: '#EF4444', width: 20, height: 20, borderRadius: '50%', fontSize: 11, display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'absolute', top: -5, left: -5, border: '2px solid #0B0D1F' }}>{totalItems}</span>}
            </button>
          </div>
        </header>
        ...
        <div style={{ maxWidth: 1100, margin: '40px auto 80px', padding: '0 24px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: 48, alignItems: 'start' }}>
            <div>
              <div style={{ borderRadius: 24, overflow: 'hidden', border: '1px solid rgba(255,77,109,0.15)', aspectRatio: '1', background: '#14162E', position: 'relative' }}>
                ...
                {p.badge && !isOutOfStock && (
                  <span style={{ position: 'absolute', top: 16, right: 16, background: 'linear-gradient(135deg,#FF4D6D,#FF9A3C)', color: 'white', padding: '6px 14px', borderRadius: 20, fontSize: 12, fontWeight: 700 }}>{p.badge}</span>
                )}
                {isOutOfStock && (
                  <div style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.55)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    <span style={{ background: '#EF4444', color: 'white', padding: '10px 24px', borderRadius: 14, fontWeight: 800, fontSize: 16 }}>نفذت الكمية</span>
                  </div>
                )}
              </div>
            </div>
            ...
            <div>
              <span style={{ fontSize: 13, color: '#FF4D6D', fontWeight: 600 }}>{p.category}</span>
              <h1 style={{ fontSize: 30, fontWeight: 800, color: 'white', margin: '6px 0 14px', lineHeight: 1.3 }}>{p.name}</h1>

              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
                {renderStars(p.rating)}
                <span style={{ color: '#EAB308', fontWeight: 700, fontSize: 15 }}>{p.rating}</span>
                <span style={{ color: '#6B7280', fontSize: 13 }}>({p.reviewsCount} تقييم)</span>
              </div>
              ...
              <div style={{ background: 'rgba(20,22,46,0.7)', border: '1px solid rgba(255,77,109,0.12)', borderRadius: 18, padding: 24, marginBottom: 24 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
                  <span style={{ fontSize: 34, fontWeight: 800, color: p.oldPrice ? '#22C55E' : '#E6E6EA' }}>{p.price} ر.س</span>
                  {p.oldPrice && <span style={{ fontSize: 18, color: '#6B7280', textDecoration: 'line-through' }}>{p.oldPrice} ر.س</span>}
              ...
              <div style={{ display: 'flex', gap: 12, marginBottom: 28 }}>
                <button
                  ...
                  style={{ flex: 1, padding: '16px 24px', borderRadius: 14, background: isOutOfStock ? '#374151' : addedIds.has(p.id) ? '#22C55E' : 'linear-gradient(135deg,#FF4D6D,#FF9A3C)', color: 'white', border: 'none', cursor: isOutOfStock ? 'not-allowed' : 'pointer', fontSize: 17, fontWeight: 800, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10 }}
                >
              ...
              <p style={{ fontSize: 15, color: '#D1D5DB', lineHeight: 1.8, margin: '0 0 24px' }}>{p.description}</p>
            </div>
          </div>
        </div>
        ...
        <div onClick={e => e.stopPropagation()} style={{ width: '100%', maxWidth: 440, background: '#10122B', borderLeft: '1px solid rgba(255,77,109,0.15)', height: '100%', display: 'flex', flexDirection: 'column', padding: 24 }}>
          ...
          {cart.length > 0 && (
            <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: 16, marginTop: 16 }}>
              ...
              <Link href="/demo/store/checkout" onClick={() => setIsCartOpen(false)} style={{ width: '100%', padding: 13, borderRadius: 12, background: 'linear-gradient(135deg,#FF4D6D,#FF9A3C)', color: 'white', border: 'none', fontWeight: 800, fontSize: 15, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, textDecoration: 'none' }}>
