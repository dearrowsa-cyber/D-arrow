...
function AdminDashboardContent() {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'theme' | 'products' | 'orders' | 'abandoned' | 'pixels' | 'coupons' | 'seo'>('overview');
  
  // SEO & Search Engine Console States
  const [seoPreviewMode, setSeoPreviewMode] = useState<'desktop' | 'mobile'>('desktop');
  const [activeSeoSubtab, setActiveSeoSubtab] = useState<'meta' | 'schema' | 'audit' | 'sitemap'>('meta');
  ...
  const [newProdImage, setNewProdImage] = useState('https://images.unsplash.com/photo-1505740420928-5e560c06d30e?auto=format&fit=crop&w=800&q=80');
  const [newProdDesc, setNewProdDesc] = useState('');
  const [newProdBadge, setNewProdBadge] = useState('جديد');
  const [newProdFeatures, setNewProdFeatures] = useState('ضمان سعودي سنتين\nشحن سريع لكافة المدن');
  ...
  const handleProductImageFile = (e: React.ChangeEvent<HTMLInputElement>, mode: 'add' | 'edit') => {
    const file = e.target.files?.[0];
    if (!file) return;
  ...
  const handleExportReport = () => {
    const rows = [
      ['رقم الطلب', 'العميل', 'المدينة', 'المبلغ الإجمالي (ر.س)', 'ضريبة القيمة المضافة 15%', 'طريقة الدفع', 'الحالة'],
      ...orders.map(o => [
        o.orderNumber,
        o.customerName,
        o.customerCity,
    ...
    link.setAttribute('download', `darrow_store_financial_report_${Date.now()}.csv`);
  ...
  const handleFirePixelTest = (eventName: string) => {
    ...
    triggerToast(`تم إرسال حدث ${eventName} بنجاح إلى (Snapchat, TikTok, Google, Meta)`);
  };
  ...
  return (
    <div className="min-h-screen bg-[#070914] text-white flex flex-col font-sans demo-store-root" dir="rtl">
      ...
      <header className="bg-[#0B0E24] border-b border-white/10 sticky top-0 z-30">
        <div className="max-w-[1550px] w-full mx-auto px-4 sm:px-8 lg:px-12">
          <div className="flex items-center justify-between h-20 gap-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-400">إدارة الهوية والمنتجات والطلبات والتسويق والإعلانات</p>
              </div>
            </div>
          ...
          <nav className="flex items-center gap-1.5 sm:gap-2 overflow-x-auto py-2.5 text-xs font-bold border-t border-white/5 no-scrollbar">
            ...
            >
              <Palette className="w-4 h-4 flex-shrink-0" style={{ color: settings.primaryColor }} />
            ...
            >
              ...
              <span className="hidden sm:inline">المنتجات ({products.length})</span>
            ...
            >
              ...
              <span className="hidden sm:inline">الطلبات ({orders.length})</span>
            ...
            >
              ...
              <span className="hidden sm:inline">كوبونات ({coupons.length})</span>
      ...
      <main className="flex-1 max-w-[1550px] w-full mx-auto px-3 sm:px-8 lg:px-12 py-4 sm:py-8 space-y-6 sm:space-y-10">
        ...
        {activeTab === 'overview' && (
          <div className="space-y-8 animate-in fade-in">
            ...
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                <span className="text-lg sm:text-xl font-extrabold text-white block">لوحة المؤشرات والتقارير المالية</span>
                <p className="text-xs text-slate-400">إحصائيات المبيعات ومتوسط السلة والضرائب (ZATCA VAT 15%)</p>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
              <div className="p-6 rounded-3xl bg-[#10132E] border border-white/10 shadow-xl space-y-2">
                ...
                <div className="text-2xl sm:text-3xl font-black text-white font-mono">
                  {totalRevenue.toLocaleString()} <span className="text-xs text-[#FF9A3C] font-normal">{settings.currency}</span>
                </div>
                <span className="text-xs text-emerald-400 font-bold block">+24.8% مقارنة بالشهر الماضي</span>
              </div>
              ...
              <div className="p-6 rounded-3xl bg-[#10132E] border border-white/10 shadow-xl space-y-2">
                ...
                <div className="text-2xl sm:text-3xl font-black text-white font-mono">
                  {totalOrdersCount} <span className="text-xs text-slate-400 font-normal">طلب</span>
                </div>
                <span className="text-xs text-emerald-400 font-bold block">معدل تحويل مرتفع 3.8%</span>
              </div>
              ...
              <div className="p-6 rounded-3xl bg-[#10132E] border border-white/10 shadow-xl space-y-2">
                ...
                <div className="text-2xl sm:text-3xl font-black text-white font-mono">
                  {averageOrderValue} <span className="text-xs text-[#FF9A3C] font-normal">{settings.currency}</span>
                </div>
                <span className="text-xs text-slate-400 block">بفضل تقسيط تمارا وتابي</span>
              </div>
              ...
              <div className="p-6 rounded-3xl bg-[#10132E] border border-white/10 shadow-xl space-y-2">
                ...
                <div className="text-2xl sm:text-3xl font-black text-white font-mono">
                  {products.length} <span className="text-xs text-slate-400 font-normal">منتج</span>
                </div>
                <span className="text-xs text-emerald-400 font-bold block">المخزون متوفر ومحدث</span>
              </div>
            </div>
                ...
                <div className="space-y-3 pt-2">
                  <div>
                    <div className="flex justify-between text-xs font-bold mb-1">
                      ...
                      <span className="text-emerald-400 font-mono">45% (8,420 {settings.currency})</span>
                    </div>
                  ...
                  <div>
                    <div className="flex justify-between text-xs font-bold mb-1">
                      ...
                      <span className="text-[#FF9A3C] font-mono">35% (6,550 {settings.currency})</span>
                    </div>
                    <div className="w-full h-2 rounded-full bg-white/10 overflow-hidden">
                      <div className="h-full bg-[#FF9A3C] rounded-full" style={{ width: '35%' }} />
                    </div>
                  </div>
                  ...
                  <div>
                    <div className="flex justify-between text-xs font-bold mb-1">
                      ...
                      <span className="text-[#FF4D6D] font-mono">15% (2,810 {settings.currency})</span>
                    </div>
                    <div className="w-full h-2 rounded-full bg-white/10 overflow-hidden">
                      <div className="h-full bg-[#FF4D6D] rounded-full" style={{ width: '15%' }} />
                    </div>
                  </div>
                  ...
                  <div>
                    <div className="flex justify-between text-xs font-bold mb-1">
                      ...
                      <span className="text-purple-400 font-mono">5% (940 {settings.currency})</span>
                    </div>
                    <div className="w-full h-2 rounded-full bg-white/10 overflow-hidden">
              ...
              <div className="p-6 rounded-3xl bg-[#10132E] border border-white/10 space-y-4">
                ...
                <div className="space-y-3 pt-2">
                  <div className="flex items-center justify-between p-3 rounded-2xl bg-white/5 border border-white/5">
                    <div className="flex items-center gap-2">
                      <span className="w-6 h-6 rounded-full bg-[#FF4D6D]/20 text-[#FF4D6D] flex items-center justify-center font-bold text-xs">1</span>
                      <span className="font-bold text-sm">الرياض</span>
                    </div>
                    <span className="font-mono font-bold text-emerald-400 text-xs">52% من إجمالي الطلبات</span>
                  </div>

                  <div className="flex items-center justify-between p-3 rounded-2xl bg-white/5 border border-white/5">
                    <div className="flex items-center gap-2">
                      <span className="w-6 h-6 rounded-full bg-[#FF9A3C]/20 text-[#FF9A3C] flex items-center justify-center font-bold text-xs">2</span>
                      <span className="font-bold text-sm">جدة ومكة المكرمة</span>
                    </div>
                    <span className="font-mono font-bold text-emerald-400 text-xs">28% من إجمالي الطلبات</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between p-3 rounded-2xl bg-white/5 border border-white/5">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-bold text-sm">المنطقة الشرقية (الدمام والخبر)</span>
                    </div>
                    <span className="font-mono font-bold text-emerald-400 text-xs">14% من إجمالي الطلبات</span>
                  </div>
                </div>
              </div>
        ...
        {activeTab === 'theme' && (
          <div className="space-y-8 animate-in fade-in max-w-6xl">
            ...
            <div className="p-6 sm:p-8 rounded-3xl bg-[#10132E] border border-white/10 space-y-6">
              <div>
                <span className="text-lg sm:text-2xl font-black text-white flex items-center gap-2 block">
                  ...
                  <span>مولد المتاجر متعدد الأنشطة بنقرة واحدة (1-Click Multi-Niche Switcher)</span>
                </span>
                <p className="text-xs sm:text-sm text-slate-300 mt-1 font-medium">
                  اختر نشاطك التجاري ليتم استبدال كامل المنتجات، الصور، الألوان، النصوص والبنرات تلقائياً وفوراً
                </p>
              </div>
                    ...
                    >
                      ...
                      <div className="p-5 space-y-2 flex-1 flex flex-col justify-between">
                        <div className="space-y-1.5">
                          <h4 className="text-base font-black text-white">{niche.name}</h4>
                          <p className="text-xs text-slate-300 leading-relaxed font-normal">{niche.description}</p>
                        </div>
                        ...
                        <button
                          ...
                          onClick={() => {
                            ...
                            triggerToast(`تم تحويل المتجر بالكامل إلى: ${niche.name} بنجاح`);
                          }}
                          style={isCurrent ? { background: `linear-gradient(135deg, ${settings.primaryColor}, ${settings.accentColor})` } : {}}
                        ...
                        >
                          <span>{isCurrent ? 'النشاط المفعّل حالياً' : 'تطبيق هذا النشاط فوراً'}</span>
                        </button>
                      </div>
                    </div>
                  );
            ...
            <div className="p-6 sm:p-8 rounded-3xl bg-[#10132E] border border-white/10 space-y-6">
              ...
              <div>
                <span className="text-lg sm:text-2xl font-black text-white flex items-center gap-2 block">
                  <Palette className="w-6 h-6" style={{ color: settings.primaryColor }} />
                  <span>استوديو تخصيص هوية وثيم وألوان المتجر</span>
                </span>
                <p className="text-xs sm:text-sm text-slate-300 mt-1 font-medium">
                  تحكم كامل في ألوان الهيدر، الخلفيات، الكروت، الفوتر، والشعار ونصوص المتجر
                </p>
              </div>

              {/* Theme Presets */}
              <div className="space-y-3 pt-2">
                <label className="text-xs sm:text-sm font-bold text-slate-200 block">اختر ثيماً ونظام ألوان متكامل جاهز:</label>
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div className="overflow-hidden">
                            <p className="font-black text-sm text-white truncate">{preset.nameAr}</p>
                            <p className="text-[11px] text-slate-400 font-mono mt-0.5">{preset.nameEn}</p>
                            <div className="flex items-center gap-1.5 mt-2">
                              <span className="w-3 h-3 rounded-full border border-white/20" style={{ backgroundColor: preset.headerBg }} title="الهيدر" />
                              <span className="w-3 h-3 rounded-full border border-white/20" style={{ backgroundColor: preset.pageBg }} title="الخلفية" />
                              <span className="w-3 h-3 rounded-full border border-white/20" style={{ backgroundColor: preset.primary }} title="اللون الأساسي" />
                            </div>
                          </div>
                        </div>
              ...
              <div className="p-5 rounded-2xl bg-[#141738] border border-white/10 space-y-4">
                <span className="font-bold text-xs text-slate-200 block">تخصيص درجات ألوان الواجهة بدقة:</span>
                
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  <div>
                    <label className="text-[11px] text-slate-300 font-bold block mb-1">لون خلفية الهيدر (Header BG)</label>
                    <div className="flex items-center gap-2 bg-[#1B1F47] p-2.5 rounded-xl border border-white/15">
                      ...
                      />
                      <span className="font-mono text-xs font-bold text-white">{settings.headerBgColor || '#0A0C1E'}</span>
                    </div>
                  </div>

                  <div>
                    <label className="text-[11px] text-slate-300 font-bold block mb-1">لون خلفية الصفحة (Page BG)</label>
                    <div className="flex items-center gap-2 bg-[#1B1F47] p-2.5 rounded-xl border border-white/15">
                      <input
                        type="color"
                        value={settings.pageBgColor || '#070914'}
                        onChange={(e) => updateSettings({ pageBgColor: e.target.value })}
                        className="w-7 h-7 rounded-lg cursor-pointer bg-transparent border-0"
                      />
                      <span className="font-mono text-xs font-bold text-white">{settings.pageBgColor || '#070914'}</span>
                    </div>
                  </div>

                  <div>
                    <label className="text-[11px] text-slate-300 font-bold block mb-1">لون خلفية الكروت (Cards BG)</label>
                    <div className="flex items-center gap-2 bg-[#1B1F47] p-2.5 rounded-xl border border-white/15">
                      <input
                        type="color"
                        value={settings.cardBgColor || '#10132E'}
                        onChange={(e) => updateSettings({ cardBgColor: e.target.value })}
                        className="w-7 h-7 rounded-lg cursor-pointer bg-transparent border-0"
                      />
                      <span className="font-mono text-xs font-bold text-white">{settings.cardBgColor || '#10132E'}</span>
                    </div>
                  </div>

                  <div>
                    <label className="text-[11px] text-slate-300 font-bold block mb-1">لون خلفية الفوتر (Footer BG)</label>
                    <div className="flex items-center gap-2 bg-[#1B1F47] p-2.5 rounded-xl border border-white/15">
                      <input
                        type="color"
                        ...
                        className="w-7 h-7 rounded-lg cursor-pointer bg-transparent border-0"
                      />
                      <span className="font-mono text-xs font-bold text-white">{settings.footerBgColor || '#050714'}</span>
                    </div>
                  </div>

                  <div>
                    <label className="text-[11px] text-slate-300 font-bold block mb-1">اللون الأساسي (Primary Color)</label>
                    <div className="flex items-center gap-2 bg-[#1B1F47] p-2.5 rounded-xl border border-white/15">
                      <input
                        type="color"
                        ...
                        className="w-7 h-7 rounded-lg cursor-pointer bg-transparent border-0"
                      />
                      <span className="font-mono text-xs font-bold text-white">{settings.primaryColor}</span>
                    </div>
                  </div>

                  <div>
                    <label className="text-[11px] text-slate-300 font-bold block mb-1">اللون الثانوي (Accent Color)</label>
                    <div className="flex items-center gap-2 bg-[#1B1F47] p-2.5 rounded-xl border border-white/15">
                      <input
                        type="color"
                        ...
                        className="w-7 h-7 rounded-lg cursor-pointer bg-transparent border-0"
                      />
                      <span className="font-mono text-xs font-bold text-white">{settings.accentColor}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4 pt-2">
                ...
                <div className="p-4 rounded-2xl bg-[#141738] border border-white/15 flex flex-col sm:flex-row items-center gap-4">
                  <div className="w-20 h-20 rounded-2xl bg-black/40 border border-white/15 p-2 flex items-center justify-center overflow-hidden flex-shrink-0 shadow-inner">
                    {settings.logoUrl ? (
                      <img src={settings.logoUrl} alt="Store Logo Preview" className="w-full h-full object-contain" />
                    ) : (
                      <span className="text-xs text-slate-500 font-bold">بدون لوجو</span>
                    )}
                  </div>

                  <div className="flex-1 space-y-2 text-center sm:text-right">
                    <p className="text-xs font-bold text-white">رفع صورة الشعار (Upload Logo)</p>
                    <p className="text-[11px] text-slate-400">يدعم صيغ PNG, JPG, WebP, SVG مع خلفية شفافة</p>
                    ...
                    <div className="flex items-center gap-2 pt-1 justify-center sm:justify-start">
                      ...
                      >
                        ...
                        <span>{isUploadingLogo ? 'جاري الرفع...' : 'اختيار صورة من جهازك'}</span>
                      </button>
                ...
                <div>
                  <label className="text-xs font-bold text-slate-300 block mb-1">اسم المتجر</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="text-xs font-bold text-slate-300 block mb-1">الشعار اللفظي للمتجر (Slogan)</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="text-xs font-bold text-slate-300 block mb-1">نص شريط الإعلانات الترويجي العلوي</label>
                  <textarea
              ...
              <div className="pt-4 border-t border-white/10 flex gap-3">
                <button
                  type="button"
                  onClick={() => triggerToast('تم حفظ وتطبيق كافة إعدادات الهوية والألوان بنجاح')}
                  style={{ background: `linear-gradient(135deg, ${settings.primaryColor}, ${settings.accentColor})` }}
        ...
        {activeTab === 'products' && (
          <div className="space-y-6 animate-in fade-in">
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                <span className="text-lg sm:text-xl font-extrabold text-white block">إدارة المنتجات والمخزون</span>
                <p className="text-xs text-slate-400">إضافة وتعديل وحذف ورفع صور المنتجات مباشرة من جهاز الكمبيوتر</p>
              </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-right text-xs">
                  ...
                  <tbody className="divide-y divide-[#382E0E] text-slate-200">
                    {products.map((prod) => (
                      <tr key={prod.id} className="hover:bg-[#241D09] transition">
                        <td className="p-4">
                          <div className="flex items-center gap-3">
                        ...
                        <td className="p-4 font-medium text-[#D1D5DB]">{prod.category}</td>
                        <td className="p-4 font-black font-mono text-[#FFE600] text-sm">{prod.price} {settings.currency}</td>
                        <td className="p-4 font-mono text-slate-500 line-through">{prod.oldPrice ? `${prod.oldPrice} ${settings.currency}` : '-'}</td>
                        <td className="p-4">
                          <span className={`px-2.5 py-1 rounded-full font-bold text-[10px] ${prod.stock < 10 ? 'bg-amber-500/20 text-amber-300 border border-amber-500/30' : 'bg-emerald-500/20 text-emerald-300 border border-emerald-500/30'}`}>
                            {prod.stock} متوفرة
                          </span>
                        </td>
                        <td className="p-4">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in">
            <div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-3xl bg-[#1C1707] border-2 border-[#FFE600] p-6 sm:p-8 space-y-5 shadow-2xl">
              <div className="flex items-center justify-between border-b border-[#382E0E] pb-4">
                <div>
                  <span className="font-black text-lg text-white block">تعديل بيانات وصفحة المنتج</span>
                  <span className="text-xs text-[#FFE600] font-mono font-bold">معرف المنتج: {editingProduct.id}</span>
                </div>
                <button onClick={() => setEditingProduct(null)} className="p-2 rounded-xl bg-[#2D250B] text-white hover:text-[#FFE600]">
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <form onSubmit={handleUpdateProduct} className="space-y-4 text-xs">
                <div>
                  <label className="block text-white font-bold mb-1.5">اسم المنتج *</label>
                  <input
                    type="text"
                    required
                    value={editProdName}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                  <div>
                    <label className="block text-white font-bold mb-1.5">القسم / التصنيف</label>
                    <input
                      type="text"
                      required
                  ...
                  <div>
                    <label className="block text-white font-bold mb-1.5">الوسم الترويجي (Badge)</label>
                    <input
                      type="text"
                      value={editProdBadge}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3.5">
                  <div>
                    <label className="block text-white font-bold mb-1.5">السعر الحالي ({settings.currency}) *</label>
                    <input
                      type="number"
                      required
                      value={editProdPrice}
                  ...
                  <div>
                    <label className="block text-white font-bold mb-1.5">السعر قبل الخصم</label>
                    <input
                      type="number"
                      value={editProdOldPrice}
                      onChange={(e) => setEditProdOldPrice(e.target.value ? Number(e.target.value) : '')}
                      placeholder="اختياري"
                  ...
                  <div>
                    <label className="block text-white font-bold mb-1.5">المخزون المتوفر (الكمية)</label>
                    <input
                      type="number"
                      value={editProdStock}
                ...
                <div className="p-4 rounded-2xl bg-[#120E04] border border-[#382E0E] space-y-3">
                  <div className="flex items-center justify-between">
                    <label className="text-white font-bold block">صورة المنتج</label>
                    <span className="text-[10px] text-[#FFE600] font-bold">يمكنك الرفع المباشر من جهازك أو وضع رابط</span>
                  </div>

                  <div className="flex items-center gap-3">
                    <div className="w-16 h-16 rounded-xl overflow-hidden bg-[#1C1707] border border-[#382E0E] flex-shrink-0">
                      {editProdImage && <img src={editProdImage} alt="Preview" className="w-full h-full object-cover" />}
                    </div>
                ...
                <div>
                  <label className="block text-white font-bold mb-1.5">وصف المنتج</label>
                  <textarea
                    rows={3}
                    value={editProdDesc}
                ...
                <div>
                  <label className="block text-white font-bold mb-1.5">المميزات والمواصفات (ميزة في كل سطر)</label>
                  <textarea
                    rows={3}
                ...
                <div className="flex items-center gap-2 p-3 rounded-xl bg-[#120E04] border border-[#382E0E]">
                  ...
                  />
                  <label htmlFor="editFlashDeal" className="text-white font-bold cursor-pointer">
        ...
        {isAddProductOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in fade-in">
            <div className="w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-3xl bg-[#1C1707] border-2 border-[#FFE600] p-6 sm:p-8 space-y-5 shadow-2xl">
              <div className="flex items-center justify-between border-b border-[#382E0E] pb-4">
                <span className="font-black text-lg text-white block">إضافة منتج جديد إلى المتجر</span>
                <button onClick={() => setIsAddProductOpen(false)} className="p-2 rounded-xl bg-[#2D250B] text-white hover:text-[#FFE600]">
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <form onSubmit={handleCreateProduct} className="space-y-4 text-xs">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                  <div>
                    <label className="block text-white font-bold mb-1.5">القسم</label>
                    <input
                      type="text"
                      required
                  ...
                  <div>
                    <label className="block text-white font-bold mb-1.5">الوسم الترويجي</label>
                    <input
                      type="text"
                      value={newProdBadge}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3.5">
                  <div>
                    <label className="block text-white font-bold mb-1.5">السعر ({settings.currency}) *</label>
                    <input
                      type="number"
                      required
                      value={newProdPrice}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setNewProdOldPrice(e.target.value ? Number(e.target.value) : '')}
                      placeholder="اختياري"
                      className="w-full !bg-[#120E04] !text-white border-2 !border-[#382E0E] focus:!border-[#FFE600] rounded-xl px-3.5 py-2.5 text-xs font-mono font-bold"
                    />
                  </div>
                  ...
                  <div>
                    <label className="block text-white font-bold mb-1.5">المخزون</label>
                    <input
                      type="number"
                      value={newProdStock}
                ...
                <div className="p-4 rounded-2xl bg-[#120E04] border border-[#382E0E] space-y-3">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="w-16 h-16 rounded-xl overflow-hidden bg-[#1C1707] border border-[#382E0E] flex-shrink-0">
                      {newProdImage && <img src={newProdImage} alt="Preview" className="w-full h-full object-cover" />}
                    </div>
                ...
                <div>
                  <label className="block text-white font-bold mb-1.5">المميزات (ميزة في كل سطر)</label>
                  <textarea
                    rows={3}
                ...
                <div className="flex items-center gap-2 p-3 rounded-xl bg-[#120E04] border border-[#382E0E]">
                  <input
                    ...
                    className="w-4 h-4 accent-[#FFE600] cursor-pointer"
                  />
                  <label htmlFor="addFlashDeal" className="text-white font-bold cursor-pointer">
        ...
        {activeTab === 'orders' && (
          <div className="space-y-6 animate-in fade-in">
            <div>
              <span className="text-lg sm:text-xl font-extrabold text-white block">إدارة الطلبات والمبيعات</span>
              <p className="text-xs text-slate-400">متابعة طلبات العملاء وتحديث حالات الشحن والتوصيل</p>
            </div>
                    ...
                    {orders.map((ord) => (
                      <tr key={ord.id} className="hover:bg-white/5 transition">
                        ...
                        <td className="p-4">
                          ...
                          <span className="text-[10px] text-slate-400 font-mono" dir="ltr">{ord.customerPhone}</span>
                        </td>
                        <td className="p-4 text-slate-300">{ord.customerCity}</td>
                        <td className="p-4 font-black font-mono text-[#FF9A3C] text-sm">{ord.total} {settings.currency}</td>
                        <td className="p-4">
                        ...
                        <td className="p-4">
                          <span className={`px-2.5 py-1 rounded-full font-bold text-[10px] ${
                            ...
                            ord.status === 'processing' ? 'bg-[#FF9A3C]/20 text-[#FF9A3C] border border-[#FF9A3C]/30' :
        ...
        {activeTab === 'abandoned' && (
          <div className="space-y-6 animate-in fade-in">
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                <span className="text-lg sm:text-xl font-extrabold text-white block">استرجاع السلات المتروكة (Abandoned Cart Recovery) 🛒</span>
                <p className="text-xs text-slate-400">إرسال تذكيرات واتساب تلقائية بخصم 10% لزيادة نسبة إتمام الشراء</p>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {abandonedCarts.map((item) => (
                <div key={item.id} className="p-5 rounded-3xl bg-[#10132E] border border-white/10 space-y-4 shadow-xl flex flex-col justify-between">
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <div>
                        <h4 className="font-extrabold text-sm text-white">{item.customerName}</h4>
                        <span className="text-[11px] text-slate-400 font-mono" dir="ltr">{item.customerPhone}</span>
                      </div>
                    ...
                    <div className="space-y-1.5 p-3 rounded-2xl bg-black/30 border border-white/5 text-xs">
                      {item.items.map((prod, idx) => (
                        <div key={idx} className="flex justify-between text-slate-300">
                          ...
                          <span className="font-mono font-bold text-white">{prod.price} {settings.currency}</span>
                        </div>
                      ))}
                      <div className="pt-2 border-t border-white/10 flex justify-between font-bold text-xs text-white">
                        <span>إجمالي السلة:</span>
                        <span className="text-[#FF9A3C] font-mono">{item.total} {settings.currency}</span>
                      </div>
                    </div>
                  </div>
                    ...
                    {item.recovered ? (
                      ...
                      <button
                        onClick={() => {
                          ...
                          triggerToast(`تم إرسال تذكير واتساب إلى ${item.customerName} مع كود RECOVER10`);
                        }}
            ...
            <div className="p-6 sm:p-8 rounded-3xl bg-[#10132E] border border-white/10 space-y-6">
              <div>
                <span className="text-lg sm:text-xl font-extrabold text-white flex items-center gap-2 block">
                  ...
                  <span>مركز الربط والتكامل الإعلاني المباشر (Multi-Channel Pixels & CAPI)</span>
                </span>
                <p className="text-xs text-slate-400 mt-1">
                  تتبع التحويلات والمبيعات بدقة 100% عبر السيرفر والمتصفح لمنصات سناب شات، تيك توك، جوجل وميتا
                </p>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="p-4 rounded-2xl bg-[#141738] border border-white/10 space-y-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className="w-7 h-7 rounded-lg bg-yellow-400/20 text-yellow-300 flex items-center justify-center font-black text-xs">👻</span>
                      <span className="font-bold text-xs text-white">Snapchat Pixel & CAPI</span>
                    </div>
                    <label className="relative inline-flex items-center cursor-pointer">
                      <input
                        type="checkbox"
                        checked={marketingPixels.snapchatActive}
                        onChange={(e) => updateMarketingPixels({ snapchatActive: e.target.checked })}
                  ...
                  <input
                    ...
                    onChange={(e) => updateMarketingPixels({ snapchatPixel: e.target.value })}
                ...
                <div className="p-4 rounded-2xl bg-[#141738] border border-white/10 space-y-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className="w-7 h-7 rounded-lg bg-pink-500/20 text-pink-300 flex items-center justify-center font-black text-xs">🎵</span>
                      <span className="font-bold text-xs text-white">TikTok Pixel & Events API</span>
                    </div>
                    <label className="relative inline-flex items-center cursor-pointer">
                      <input
                        type="checkbox"
                        checked={marketingPixels.tiktokActive}
                        onChange={(e) => updateMarketingPixels({ tiktokActive: e.target.checked })}
                        className="sr-only peer"
                      />
                      <div className="w-8 h-4 bg-white/10 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-3 after:w-3 after:transition-all peer-checked:bg-emerald-500"></div>
                    </label>
                  ...
                  <input
                    ...
                    onChange={(e) => updateMarketingPixels({ tiktokPixel: e.target.value })}
                ...
                <div className="p-4 rounded-2xl bg-[#141738] border border-white/10 space-y-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className="w-7 h-7 rounded-lg bg-blue-500/20 text-blue-300 flex items-center justify-center font-black text-xs">🔍</span>
                      <span className="font-bold text-xs text-white">Google Ads & GA4 Tracking</span>
                    </div>
                    <label className="relative inline-flex items-center cursor-pointer">
                      <input
                        type="checkbox"
                        checked={marketingPixels.googleAdsActive}
                        onChange={(e) => updateMarketingPixels({ googleAdsActive: e.target.checked })}
                        className="sr-only peer"
                      />
                      <div className="w-8 h-4 bg-white/10 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-3 after:w-3 after:transition-all peer-checked:bg-emerald-500"></div>
                    </label>
                  ...
                  <input
                    ...
                    onChange={(e) => updateMarketingPixels({ googleAdsId: e.target.value })}
                ...
                <div className="p-4 rounded-2xl bg-[#141738] border border-white/10 space-y-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <span className="w-7 h-7 rounded-lg bg-indigo-500/20 text-indigo-300 flex items-center justify-center font-black text-xs">🌐</span>
                      <span className="font-bold text-xs text-white">Meta (FB & IG) Pixel</span>
                    </div>
                    <label className="relative inline-flex items-center cursor-pointer">
                      <input
                        type="checkbox"
                        checked={marketingPixels.metaActive}
                        onChange={(e) => updateMarketingPixels({ metaActive: e.target.checked })}
                        className="sr-only peer"
                      />
                      <div className="w-8 h-4 bg-white/10 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-3 after:w-3 after:transition-all peer-checked:bg-emerald-500"></div>
                    </label>
                  ...
                  <input
                    ...
                    onChange={(e) => updateMarketingPixels({ metaPixel: e.target.value })}
              ...
              <div className="p-5 rounded-2xl bg-black/40 border border-white/15 space-y-3">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-xs text-white flex items-center gap-2">
                    ...
                    <span>محاكي اختبار إرسال الأحداث اللحظي (Test Live Events):</span>
                  </span>
                  {lastFiredEvent && (
                ...
                <div className="flex flex-wrap items-center gap-2 pt-1">
                  <button
                    onClick={() => handleFirePixelTest('PageView (زيارة الصفحة)')}
                  ...
                  <button
                    onClick={() => handleFirePixelTest('ViewContent (مشاهدة المنتج)')}
                    className="px-3 py-2 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-xs font-bold transition cursor-pointer"
                  >
                    👁️ إرسال ViewContent
                  </button>
                  <button
                    onClick={() => handleFirePixelTest('AddToCart (إضافة للسلة)')}
                    className="px-3 py-2 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-xs font-bold transition cursor-pointer"
                  >
                    🛒 إرسال AddToCart
                  </button>
        ...
        {activeTab === 'coupons' && (
          <div className="space-y-6 animate-in fade-in max-w-4xl">
            <div className="flex items-center justify-between">
              <div>
                <span className="text-lg sm:text-xl font-extrabold text-white block">كوبونات وقسائم الخصم الترويجية</span>
                <p className="text-xs text-slate-400">إنشاء وتفعيل أكواد الخصم للعملاء وحملات التسويق</p>
              </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {coupons.map((coupon) => (
                <div key={coupon.code} className="p-5 rounded-3xl bg-[#10132E] border border-white/10 space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      onClick={() => {
                        ...
                        triggerToast(coupon.active ? 'تم إيقاف الكوبون' : 'تم تفعيل الكوبون');
                      }}
                  ...
                  <div className="text-xs text-slate-300 space-y-1">
                    <p>نسبة الخصم: <strong className="text-emerald-400 font-mono">{coupon.discountPct}%</strong></p>
                    <p>الحد الأدنى للطلب: <strong className="font-mono">{coupon.minSpend} {settings.currency}</strong></p>
                  </div>
                </div>
              ))}
            </div>
        ...
        {activeTab === 'seo' && (
          <div className="space-y-8 animate-in fade-in max-w-5xl">
            ...
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                <div className="flex items-center gap-2">
                  <span className="text-lg sm:text-2xl font-black text-white block">مركز تحسين محركات البحث والسيو الذكي (SEO)</span>
                  <span className="px-3 py-1 rounded-full bg-[#FFE600]/15 border border-[#FFE600]/40 text-[#FFE600] font-black text-[11px]">
                    Google Optimized
                  </span>
                </div>
                <p className="text-xs text-slate-400 mt-1">
                  إدارة وسوم الميتا، معاينة بطاقة بحث جوجل الحية، خريطة الموقع Sitemap، وبيانات Schema.org المنظمة
                </p>
              </div>
              ...
              <div className="flex items-center gap-2">
                <button
                  onClick={() => {
                    ...
                    setGooglePingStatus('جاري إرسال إشعار الأرشفة الفورية لعناكب Googlebot...');
                    setTimeout(() => {
                      setIsPingingGoogle(false);
                      setGooglePingStatus('تم استلام إشعار الأرشفة بنجاح من Google Search Index API (HTTP 200)');
                      triggerToast('تم إرسال إشعار الأرشفة إلى محرك بحث جوجل بنجاح');
                    }, 1500);
                  }}
                ...
                >
                  ...
                  <span>{isPingingGoogle ? 'جاري الأرشفة...' : 'طلب أرشفة فورية لجوجل'}</span>
                </button>

                <button
                  onClick={() => triggerToast('تم حفظ ونشر إعدادات السيو والوسوم عالمياً')}
            ...
            {googlePingStatus && (
              <div className="p-3.5 rounded-2xl bg-[#FFE600]/10 border border-[#FFE600]/30 text-[#FFE600] text-xs font-bold flex items-center justify-between">
                ...
                <button onClick={() => setGooglePingStatus(null)} className="text-slate-400 hover:text-white">
                  <X className="w-4 h-4" />
                </button>
              </div>
            )}
            ...
            {activeSeoSubtab === 'meta' && (
              <div className="space-y-6 animate-in fade-in">
                ...
                <div className="p-6 rounded-3xl bg-[#0B0E14] border-2 border-white/15 space-y-4 shadow-2xl">
                  <div className="flex items-center justify-between border-b border-white/10 pb-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-bold text-xs text-slate-300">معاينة مباشرة لشكل متجرك في نتائج بحث Google (Live SERP Preview)</span>
                    </div>
                    ...
                    <div className="flex items-center bg-[#151928] p-1 rounded-xl border border-white/10 text-xs">
                      <button
                        ...
                        className={`px-3 py-1 rounded-lg transition font-bold ${
                          seoPreviewMode === 'desktop' ? 'bg-[#FFE600] text-black font-black' : 'text-slate-400 hover:text-white'
                        }`}
                      >
                        كمبيوتر (Desktop)
                      </button>
                      <button
                        ...
                        className={`px-3 py-1 rounded-lg transition font-bold ${
                          seoPreviewMode === 'mobile' ? 'bg-[#FFE600] text-black font-black' : 'text-slate-400 hover:text-white'
                        }`}
                      >
                        جوال (Mobile)
                      </button>
                  ...
                  <div className={`p-4 rounded-2xl bg-[#171A21] border border-white/10 space-y-2 ${seoPreviewMode === 'mobile' ? 'max-w-md' : 'w-full'}`} dir="rtl">
                    {/* URL Breadcrumbs */}
                    <div className="flex items-center gap-1.5 text-[11px] text-[#BDC1C6]">
                      <div className="w-4 h-4 rounded-full bg-slate-700 flex items-center justify-center text-[9px] text-white">🌐</div>
                      <span className="text-[#BDC1C6] font-mono">https://d-arrow.com</span>
                      <span className="text-slate-500">›</span>
                      <span className="text-[#BDC1C6] font-mono">store</span>
                      <span className="text-slate-500">›</span>
                      <span className="text-[#BDC1C6]">{settings.storeName}</span>
                    </div>
                    ...
                    <h3 className="text-base sm:text-lg font-bold text-[#8AB4F8] hover:underline cursor-pointer leading-snug">
                      {settings.seoMetaTitle || `${settings.storeName} | المتجر الإلكتروني السعودي الفاخر`}
                    </h3>
                    ...
                    <div className="flex flex-wrap items-center gap-2 text-[11px] text-[#9AA0A6] font-mono">
                      <div className="flex items-center text-amber-400">
                        <span>★★★★★</span>
                        <span className="text-slate-300 font-bold mr-1">4.9</span>
                      </div>
                      <span>·</span>
                      <span className="text-emerald-400 font-bold">متوفر في المخزون (In Stock)</span>
                      <span>·</span>
                      <span className="text-slate-300">الضمان: سنتين معتمد</span>
                      <span>·</span>
                      <span className="text-slate-300">الشحن: فوري لجميع المدن 🇸🇦</span>
                    </div>

                    {/* Meta Description */}
                    <p className="text-xs text-[#BDC1C6] leading-relaxed">
                      {settings.seoMetaDescription || settings.storeSlogan || 'تسوق الآن من متجرنا المعتمد مع أفضل العروض الحصرية وضمان شامل وشحن سريع لجميع مدن المملكة.'}
                    </p>
                  </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2 text-xs">
                    <div className="p-3 rounded-xl bg-[#121530] border border-white/10 flex items-center justify-between">
                      <span className="text-slate-300 font-bold">طول عنوان السيو (Title):</span>
                      <div className="flex items-center gap-2">
                    ...
                    <div className="p-3 rounded-xl bg-[#121530] border border-white/10 flex items-center justify-between">
                      <span className="text-slate-300 font-bold">طول الوصف التعريفي (Description):</span>
                      <div className="flex items-center gap-2">
                        <span className="font-mono font-black text-[#FFE600]">
                ...
                <div className="p-6 rounded-3xl bg-[#10132E] border border-white/10 space-y-5">
                  <span className="font-black text-sm text-white block">⚙️ تخصيص وسوم الميتا ومحركات البحث للمتجر:</span>
                  ...
                  <div className="space-y-4 text-xs">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => updateSettings({ seoMetaTitle: e.target.value })}
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        onChange={(e) => updateSettings({ seoMetaDescription: e.target.value })}
                    ...
                    <div>
                      ...
                      <div className="p-3 rounded-2xl bg-[#15193B] border border-white/20 space-y-2.5">
                        <div className="flex flex-wrap gap-2">
                          {(settings.seoKeywords || 'متجر إلكتروني سعودي, أجهزة ذكية, سماعات لاسلكية, عروض السعودية').split(',').map((kw, idx) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <button
                                  type="button"
                                  onClick={() => {
                                    const currentList = (settings.seoKeywords || '').split(',').map(k => k.trim()).filter(Boolean);
                                    const nextList = currentList.filter(k => k !== trimmed);
                                    updateSettings({ seoKeywords: nextList.join(', ') });
                                  }}
                          ...
                          <input
                            ...
                            onKeyDown={(e) => {
                              if (e.key === 'Enter' && newKeywordInput.trim()) {
                                e.preventDefault();
                                const current = settings.seoKeywords ? settings.seoKeywords + ', ' : '';
                                updateSettings({ seoKeywords: current + newKeywordInput.trim() });
                                setNewKeywordInput('');
                              }
                            }}
                        ...
                        <div className="flex flex-wrap items-center gap-1.5 pt-1 text-[11px]">
                          ...
                          {['شحن مجاني السعودية', 'ضمان سنتين', 'دفع عند الاستلام', 'أبل باي ومدى', 'عروض اليوم الوطني'].map((sug) => (
                            <button
                              key={sug}
                              type="button"
                              onClick={() => {
                                ...
                                if (!current.includes(sug)) {
                                  updateSettings({ seoKeywords: current + sug });
                                }
                              }}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
                      <div>
                        <label className="text-slate-300 font-bold block mb-1">الرابط المعتمد (Canonical URL)</label>
                        <input
                          type="text"
                          value={settings.seoCanonicalUrl || 'https://d-arrow.com/demo/store'}
                          onChange={(e) => updateSettings({ seoCanonicalUrl: e.target.value })}
                          className="w-full !bg-[#15193B] !text-white border !border-white/20 rounded-xl px-3 py-2 text-xs font-mono"
                        />
                      </div>

                      <div>
                        <label className="text-slate-300 font-bold block mb-1">توجيه الفهرسة (Robots Meta)</label>
                        <select
                          value={settings.seoRobotsIndex ? 'index' : 'noindex'}
                          onChange={(e) => updateSettings({ seoRobotsIndex: e.target.value === 'index' })}
                          className="w-full !bg-[#15193B] !text-white border !border-white/20 rounded-xl px-3 py-2 text-xs font-bold"
                        >
                          <option value="index">index, follow (السماح لجميع محركات البحث بالأرشفة الفورية - مستحسن)</option>
                          <option value="noindex">noindex, nofollow (إيقاف الأرشفة مؤقتاً)</option>
                        </select>
                      </div>
                    </div>
            ...
            {activeSeoSubtab === 'schema' && (
              <div className="space-y-6 animate-in fade-in">
                <div className="p-6 rounded-3xl bg-[#10132E] border border-white/10 space-y-4">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="text-xs text-slate-400">
                        بيانات مهيكلة تفهمها خوارزميات جوجل والذكاء الاصطناعي لعرض النجوم والأسعار وحالة المخزون
                      </p>
                    </div>
                      ...
                      <button
                        onClick={() => {
                          const schemaJSON = JSON.stringify({
                            ...
                            "paymentAccepted": "Mada, Apple Pay, Visa, MasterCard, Cash on Delivery",
                            "priceRange": "$$",
                            "address": {
                              "@type": "PostalAddress",
                              "addressCountry": "SA",
                          ...
                          triggerToast('تم نسخ كود Schema.org JSON-LD إلى الحافظة بنجاح');
                        }}
                  ...
                  {/* Schema Code Block */}
                  <div className="p-4 rounded-2xl bg-[#090C19] border border-white/15 font-mono text-[11px] text-emerald-400 overflow-x-auto" dir="ltr">
                    <pre>
              ...
              <div className="space-y-6 animate-in fade-in">
                ...
                <div className="p-6 rounded-3xl bg-gradient-to-r from-[#1C1707] to-[#120E04] border-2 border-[#FFE600]/50 flex flex-wrap items-center justify-between gap-4">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h4 className="text-base sm:text-lg font-black text-white">درجة توافق السيو لمحركات البحث: ممتازة جداً</h4>
                      <p className="text-xs text-[#FFE600]">المتجر مهيأ بالكامل ومطابق لجميع معايير تجربة المستخدم وأرشفة جوجل الفورية</p>
                    </div>
                  </div>

                  <div className="text-xs text-right font-bold text-slate-300">
                    <p>سرعة التحميل: <span className="text-emerald-400 font-mono font-black">0.4s (A+)</span></p>
                    <p>توافق الشاشات: <span className="text-emerald-400 font-mono font-black">100% Mobile Ready</span></p>
                  </div>
                </div>

                {/* Audit Checklist */}
                <div className="p-6 rounded-3xl bg-[#10132E] border border-white/10 space-y-4">
                  <span className="font-black text-sm text-white block">📋 تقرير الفحص الشامل للسيو والتهيئة التقنية:</span>
                  ...
                  <div className="space-y-2.5 text-xs font-bold">
                    <div className="p-3 rounded-2xl bg-[#15193B] border border-emerald-500/30 flex items-center justify-between text-slate-200">
                      <div className="flex items-center gap-2">
                        ...
                        <span>وسم العنوان (Meta Title): محدد ومثالي الطول (54 حرفاً) ويحتوي على الكلمات المستهدفة.</span>
                      </div>
                      <span className="text-emerald-400 font-mono font-black">ناجح ✓</span>
                    </div>

                    <div className="p-3 rounded-2xl bg-[#15193B] border border-emerald-500/30 flex items-center justify-between text-slate-200">
                      <div className="flex items-center gap-2">
                        <CheckCircle2 className="w-4 h-4 text-emerald-400" />
                        <span>الوصف التعريفي (Meta Description): جذاب وشامل ويشمل ميزة الشحن والضمان في السعودية.</span>
                      </div>
                      <span className="text-emerald-400 font-mono font-black">ناجح ✓</span>
                    </div>
                    ...
                    <div className="p-3 rounded-2xl bg-[#15193B] border border-emerald-500/30 flex items-center justify-between text-slate-200">
                      <div className="flex items-center gap-2">
                        <CheckCircle2 className="w-4 h-4 text-emerald-400" />
                        <span>البيانات المنظمة (Schema JSON-LD): مفعلة وتدعم بطاقات النجوم والأسعار بالريال السعودي.</span>
                      </div>
                      <span className="text-emerald-400 font-mono font-black">ناجح ✓</span>
                    </div>
                    ...
                    <div className="p-3 rounded-2xl bg-[#15193B] border border-emerald-500/30 flex items-center justify-between text-slate-200">
                      <div className="flex items-center gap-2">
                        <CheckCircle2 className="w-4 h-4 text-emerald-400" />
                        <span>خريطة الموقع التلقائية (Sitemap.xml): متصلة ومحدثة ديناميكياً بجميع روابط المنتجات.</span>
                      </div>
                      <span className="text-emerald-400 font-mono font-black">ناجح ✓</span>
                    </div>
                    ...
                    <div className="p-3 rounded-2xl bg-[#15193B] border border-emerald-500/30 flex items-center justify-between text-slate-200">
                      <div className="flex items-center gap-2">
                        <CheckCircle2 className="w-4 h-4 text-emerald-400" />
                        <span>ملف توجيه الروبوتات (Robots.txt): مهيأ للسماح لعناكب Googlebot و Bingbot.</span>
                      </div>
                      <span className="text-emerald-400 font-mono font-black">ناجح ✓</span>
                    </div>
                    ...
                    <div className="p-3 rounded-2xl bg-[#15193B] border border-emerald-500/30 flex items-center justify-between text-slate-200">
                      <div className="flex items-center gap-2">
                        <CheckCircle2 className="w-4 h-4 text-emerald-400" />
                        <span>وسوم OpenGraph و Twitter Cards: مفعلة وتظهر صورة وعنوان المتجر عند مشاركة الرابط على الواتساب.</span>
                      </div>
                      <span className="text-emerald-400 font-mono font-black">ناجح ✓</span>
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <div className="p-6 rounded-3xl bg-[#10132E] border border-white/10 space-y-3">
                    <div className="flex items-center justify-between border-b border-white/10 pb-2.5">
                      <span className="font-bold text-xs sm:text-sm text-white">خريطة الموقع (Sitemap.xml)</span>
                      <span className="text-[11px] font-mono text-emerald-400 font-bold">200 OK ✓</span>
                    </div>
                    <div className="p-3.5 rounded-2xl bg-[#090C19] border border-white/10 font-mono text-[11px] text-slate-300 overflow-x-auto max-h-60" dir="ltr">
                      <pre>
...
${products.map(p => `  <url>
    ...
    <priority>0.8</priority>
                  ...
                  <div className="p-6 rounded-3xl bg-[#10132E] border border-white/10 space-y-3">
                    <div className="flex items-center justify-between border-b border-white/10 pb-2.5">
                      <span className="font-bold text-xs sm:text-sm text-white">ملف توجيه العناكب (Robots.txt)</span>
                      <span className="text-[11px] font-mono text-emerald-400 font-bold">200 OK ✓</span>
                    </div>
                    <div className="p-3.5 rounded-2xl bg-[#090C19] border border-white/10 font-mono text-[11px] text-slate-300 overflow-x-auto max-h-60" dir="ltr">
                      <pre>
        ...
        {isAddCouponOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in">
            <div className="w-full max-w-md rounded-3xl bg-[#0F122B] border border-white/15 p-6 space-y-4 shadow-2xl">
              <div className="flex items-center justify-between border-b border-white/10 pb-3">
                <span className="font-black text-base text-white block">إنشاء كوبون خصم جديد</span>
              ...
              <form onSubmit={handleCreateCoupon} className="space-y-3 text-xs">
                <div>
                  <label className="block text-slate-300 font-bold mb-1">كود الخصم (Promo Code) *</label>
                  <input
                    type="text"
                    required
                    value={newCouponCode}
                ...
                <div>
                  <label className="block text-slate-300 font-bold mb-1">نسبة الخصم المئوية (%) *</label>
                  <input
                    type="number"
                    required
                    min={1}
                ...
                <div>
                  <label className="block text-slate-300 font-bold mb-1">الحد الأدنى لقيمة السلة (ر.س)</label>
                  <input
                    type="number"
