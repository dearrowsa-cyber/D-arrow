...
async function main() {
  try {
    ...
    console.log('Containers count:', Array.isArray(containersRes.data) ? containersRes.data.length : containersRes);
    if (Array.isArray(containersRes.data)) {
      containersRes.data.forEach(c => {
        console.log(`- Names: ${c.Names.join(', ')} | State: ${c.State} | Status: ${c.Status}`);
      });
    }
  } catch (err) {
