...
export default function ServiceCard({ service, index }: ServiceCardProps) {
  ...
  const resolveBackgroundImage = () => {
    ...
    const fallbackName = service.titleKey
      ? service.titleKey.replace(/[^a-z0-9]+/gi, "-")
      : "service-image";
  ...
  const serviceId = (() => {
    const id =
      (service as any).id ||
      (service.titleKey ? service.titleKey.replace("_title", "") : "");
    ...
    if (enTitle.includes("advertising") || enTitle.includes("campaign"))
      return "dm_advertising";
    ...
    if (enTitle.includes("property management") || enTitle.includes("sales"))
    ...
    if (enTitle.includes("app") && !enTitle.includes("appraisal"))
      return "id_apps";
    if (enTitle.includes("website") || enTitle.includes("web"))
      return "id_website";
  ...
  return (
    ...
    >
      ...
      >
          ...
          <div
            className={`absolute inset-0 bg-cover ${
              ((service as any).title || t(service.titleKey))
                ?.toLowerCase()
                .includes("naming") ||
              ((service as any).title || t(service.titleKey))?.includes(
                "تسميات",
              ) ||
        ...
        <div
          className={`${styles.cardBody} flex flex-col flex-1 min-w-0 w-full p-4 pt-9 pb-6 md:p-6 md:pt-12 md:pb-8 relative z-10 bg-[#0A0D1E]`}
        >
