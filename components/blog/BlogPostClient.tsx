...
export default function BlogPostClient({ post }: BlogPostClientProps) {
  ...
  const getCategoryIcon = (cat: string) => {
    ...
    if (c.includes("commerce") || c.includes("e-com"))
      return <ShoppingCart size={56} />;
    ...
    if (c.includes("market") || c.includes("digital"))
      return <TrendingUp size={56} />;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-[#0a0e27] to-[#14162E] pt-32 pb-20 blog-post-container">
      <div
        className="container mx-auto px-4 md:px-6 lg:px-12 max-w-5xl"
        dir={lang === "ar" ? "rtl" : "ltr"}
      >
        {/* Back Button */}
        <Link
        ...
        <div className="mb-10">
          <div className="flex flex-wrap items-center gap-2 mb-4">
            ...
            {tags.map((tag, i) => (
              <span
                key={i}
                className="px-3 py-1 bg-[#FF9A3C]/10 text-[#FF9A3C] text-xs font-medium rounded-full border border-[#FF9A3C]/20"
              >
                #{tag}
              </span>
            ))}
          </div>
          <h1 className="main-title font-bold text-white mb-6">{title}</h1>
          ...
          <div className="flex flex-wrap items-center gap-6 text-gray-400 text-sm border-b border-gray-800 pb-8">
            ...
            <div className="flex items-center gap-2">
              ...
              <span>
                {post.readTime} {t("blogReadTime") || "min read"}
              </span>
            </div>
          </div>
        </div>
