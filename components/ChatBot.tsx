  ...
  return (
    <>
      ...
      >
        ...
        {!isOpen && (
          <div className={`hidden sm:flex absolute top-1/2 -translate-y-1/2 whitespace-nowrap bg-gradient-to-r from-[#14162E] to-[#1a1c3e] border border-white/10 text-white px-2 sm:px-4 py-1.5 sm:py-2.5 rounded-2xl shadow-[0_0_20px_rgba(255,77,109,0.3)] text-[10px] sm:text-sm items-center gap-1.5 sm:gap-2.5 ${isRTL ? 'left-[120%] rounded-bl-sm' : 'right-[120%] rounded-br-sm'} animate-float-gentle [animation-delay:0.5s] pointer-events-none transition-all duration-300 before:absolute before:content-[''] before:top-1/2 before:-translate-y-1/2 before:border-8 before:border-transparent ${isRTL ? 'before:border-r-[#14162E] before:-left-[15px]' : 'before:border-l-[#1a1c3e] before:-right-[15px]'}`}>
            ...
            <span className="font-bold tracking-wide">
              {chatLanguage === 'ar' ? 'مساعد ' : 'D-Arrow '} <span className="bg-gradient-to-r from-[#FF9A3C] to-[#FF4D6D] bg-clip-text text-transparent">{chatLanguage === 'ar' ? 'دي أرو الذكي' : 'Smart Assistant'}</span>
            </span>
          </div>
        )}
