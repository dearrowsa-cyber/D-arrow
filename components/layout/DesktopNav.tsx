...
export default function DesktopNav({ lang, t }: { lang: string; t: any }) {
  ...
  return (
    <nav className="hidden lg:flex gap-1 items-center flex-1 justify-center px-2 xl:px-4">
      <Link href="/" className="nav-link-hover flex items-center justify-center !text-sm xl:!text-base font-medium px-3 py-2 whitespace-nowrap">
      ...
      >
        ...
        >
          <div className="flex flex-col gap-2">
            <Link href="/services?category=digital-marketing" className="dropdown-item-hover flex items-center gap-3 p-2.5 rounded-md hover:bg-[rgba(255,77,109,0.15)] transition whitespace-nowrap">
              <img src="/icon/services-icon/digital_marketing_promotion.png" alt="Digital Marketing" className="w-12 h-8 rounded-md object-cover flex-shrink-0" />
              <div className={lang === 'ar' ? 'text-right' : ''}>
                <h4 className="text-xs xl:text-sm font-semibold">{t('digitalMarketingHeader')}</h4>
              </div>
            </Link>
            <Link href="/services?category=innovation-development" className="dropdown-item-hover flex items-center gap-3 p-2.5 rounded-md hover:bg-[rgba(255,77,109,0.15)] transition whitespace-nowrap">
              <img src="/icon/services-icon/creative_digital_design.png" alt="Innovation" className="w-12 h-8 rounded-md object-cover flex-shrink-0" />
              <div className={lang === 'ar' ? 'text-right' : ''}>
                <h4 className="text-xs xl:text-sm font-semibold">{t('innovationHeader')}</h4>
              </div>
            </Link>
            <Link href="/services?category=real-estate" className="dropdown-item-hover flex items-center gap-3 p-2.5 rounded-md hover:bg-[rgba(255,77,109,0.15)] transition whitespace-nowrap">
              <img src="/icon/services-icon/real_estate_marketing.png" alt="Real Estate" className="w-12 h-8 rounded-md object-cover flex-shrink-0" />
              <div className={lang === 'ar' ? 'text-right' : ''}>
                <h4 className="text-xs xl:text-sm font-semibold">{t('realEstateHeader')}</h4>
              </div>
            </Link>
          </div>
        </div>
      ...
      <Link href="/pricing" className="nav-link-hover flex items-center justify-center !text-sm xl:!text-base font-medium px-3 py-2 whitespace-nowrap">
        <span className="nav-text">{t('packages')}</span>
      </Link>
      <Link href="/why-us" className="nav-link-hover flex items-center justify-center !text-sm xl:!text-base font-medium px-3 py-2 whitespace-nowrap">
        <span className="nav-text">{t('ourCommitment')}</span>
      </Link>
      <Link href="/provisions" className="nav-link-hover flex items-center justify-center !text-sm xl:!text-base font-medium px-3 py-2 whitespace-nowrap">
        <span className="nav-text">{t('portfolio')}</span>
      </Link>
      <Link href="/store" className="nav-link-hover flex items-center justify-center !text-sm xl:!text-base font-medium px-3 py-2 whitespace-nowrap">
        <span className="nav-text">{lang === 'ar' ? 'المتجر' : 'Store'}</span>
      </Link>
      <Link href="/blog" className="nav-link-hover flex items-center justify-center !text-sm xl:!text-base font-medium px-3 py-2 whitespace-nowrap">
        <span className="nav-text">{t('blog')}</span>
      </Link>

      {/* JOIN AS INFLUENCER LINK */}
      <Link href="/influencer" className="nav-link-hover flex items-center justify-center gap-2 !text-sm xl:!text-base font-medium px-3 py-2 whitespace-nowrap group">
        <Star size={18} className="text-[#FF9A3C] group-hover:scale-110 transition-transform drop-shadow-[0_0_8px_rgba(255,154,60,0.5)]" fill="currentColor" />
        <span className="nav-text">{lang === 'ar' ? 'انضم كمؤثر' : 'Join as Influencer'}</span>
      </Link>
      
      <Link href="/contact" className="nav-link-hover flex items-center justify-center !text-sm xl:!text-base font-medium px-3 py-2 whitespace-nowrap">
