...
export default function WhatsAppWidget() {
  ...
  return (
    <div className={`fixed bottom-6 z-40 w-16 h-16 rounded-full shadow-2xl transition-all hover:scale-110 active:scale-90 flex items-center justify-center !bg-transparent text-white ${isRTL ? 'right-6' : 'left-6'}`}>
      ...
      >
        ...
        <svg width="0" height="0">
          <linearGradient id="wa-gradient" x1="0%" y1="0%" x2="100%" y2="100%">
            <stop stopColor="#FF9A3C" offset="0%" />
      ...
      <div className={`hidden sm:flex absolute top-1/2 -translate-y-1/2 whitespace-nowrap bg-gradient-to-r from-[#14162E] to-[#1a1c3e] border border-white/10 text-white px-2 sm:px-4 py-1.5 sm:py-2.5 rounded-2xl shadow-[0_0_20px_rgba(255,77,109,0.3)] text-[10px] sm:text-sm items-center gap-1.5 sm:gap-2.5 ${isRTL ? 'right-[120%] rounded-br-sm' : 'left-[120%] rounded-bl-sm'} animate-float-gentle [animation-delay:0.5s] pointer-events-none transition-all duration-300 before:absolute before:content-[''] before:top-1/2 before:-translate-y-1/2 before:border-8 before:border-transparent ${isRTL ? 'before:border-l-[#1a1c3e] before:-right-[15px]' : 'before:border-r-[#14162E] before:-left-[15px]'}`}>
