...
export default function DynamicCTA({ type }: DynamicCTAProps) {
    ...
    switch (type) {
      case "seo":
        return {
          title:
            lang === "ar"
              ? "هل تريد تصدر نتائج بحث جوجل؟"
              : "Want to Dominate Google Search Results?",
          desc:
            lang === "ar"
      ...
      case "smm":
        return {
          title:
            lang === "ar"
              ? "اجعل علامتك التجارية تتصدر منصات التواصل"
              : "Make Your Brand Trend on Social Media",
          desc:
            lang === "ar"
          ...
          btn:
            lang === "ar"
              ? "تواصل مع خبراء السوشيال"
              : "Talk to Social Experts",
          link: "/services/smm",
      ...
      case "web":
        return {
          title:
            lang === "ar"
              ? "موقعك الإلكتروني هو واجهتك الرقمية"
  ...
  return (
    <div
      className={`mt-16 mb-8 rounded-2xl border ${content.border} bg-gradient-to-br ${content.gradient} p-8 md:p-10 relative overflow-hidden shadow-xl`}
    >
      {/* Decorative blobs */}
      ...
      <div className="relative z-10 flex flex-col md:flex-row items-center gap-8 justify-between">
        ...
        >
          ...
          <p className="text-gray-300 md:text-lg max-w-2xl">{content.desc}</p>
        </div>
