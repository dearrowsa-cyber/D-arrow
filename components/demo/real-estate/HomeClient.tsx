...
const TYPE_FILTERS: Array<PropertyType | "all"> = [
  "all",
  "villa",
  "apartment",
  "townhouse",
...
export default function RealEstateHome() {
  ...
  const [aiChat, setAiChat] = useState<
    Array<{ role: "bot" | "user"; text: string; matchedPropertyId?: string }>
  >([
    {
      role: "bot",
    ...
    () =>
      properties.filter((p) => {
        ...
        if (query.trim()) {
          const q = query.trim();
          return (
            p.title.includes(q) || p.district.includes(q) || p.city.includes(q)
          );
        }
        return true;
      }),
  ...
  const handleAiSubmit = (e: React.FormEvent) => {
    ...
    let bestMatch =
      properties.find((p) => {
        ...
        if (qLower.includes("استثمار") && p.estimatedROI && p.estimatedROI > 7)
          return true;
        return false;
      }) || properties[0];
              ...
              <div className="re-search-row">
                ...
                <div>
                  <select
                    value={dealFilter}
                    onChange={(e) =>
                      setDealFilter(e.target.value as "all" | "sale" | "rent")
                    }
                    className="re-select"
                  >
                    {DEAL_FILTERS.map((d) => (
            ...
            {stats.map((s, idx) => {
              ...
              return (
                ...
                >
                  ...
                  <div
                    style={{ fontSize: "0.85rem", color: "var(--re-muted)" }}
                  >
                    {s.label}
                  </div>
                </div>
      ...
      <section className="re-container" style={{ padding: "3.5rem 1.25rem" }}>
        ...
        >
          <div>
            ...
            <p style={{ color: "var(--re-muted)", fontSize: "0.95rem" }}>
              تصفح أرقى الفلل والشقق المعتمدة برخصة فال في المملكة
            </p>
          </div>
          <div
            style={{
            ...
            <div key={p.id} className="re-card">
              <div className="re-card-img-wrap">
                ...
                <div className="re-card-badge-top">
                  <span className="re-pill re-pill-primary">
                    {p.listingType === "sale" ? "للبيع" : "للإيجار"}
                  </span>
                  {p.tourScenes && p.tourScenes.length > 0 && (
                    <span
                      className="re-pill re-pill-dark"
                      style={{
                        display: "inline-flex",
                        alignItems: "center",
                        gap: "0.3rem",
              ...
              <div className="re-card-body">
                ...
                <div className="re-card-specs">
                  ...
                  <div className="re-card-spec-item">
                    <BedDouble size={16} />
                    <span>
                      {p.bedrooms > 0 ? `${p.bedrooms} غرف` : "مفتوح"}
                    </span>
                  </div>
                  <div className="re-card-spec-item">
                    <Bath size={16} />
        ...
        <div className="re-modal-overlay" onClick={() => setIsAiOpen(false)}>
          <div className="re-ai-modal" onClick={(e) => e.stopPropagation()}>
              ...
              >
                ...
                <div>
                  ...
                  <div style={{ fontSize: "0.75rem", opacity: 0.9 }}>
                    مساعدك الفوري لاختيار أفضل عقار
                  </div>
                </div>
              </div>
              <button
            ...
            <form
              ...
              style={{
                padding: "1rem",
                borderTop: "1px solid var(--re-border)",
                display: "flex",
                gap: "0.5rem",
              }}
            >
