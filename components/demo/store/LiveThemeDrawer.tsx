...
export default function LiveThemeDrawer() {
  ...
  return (
    <div className="fixed inset-0 z-50 pointer-events-none flex justify-end" dir="rtl">
        ...
        <div className="w-full h-full bg-[#161205]/95 backdrop-blur-2xl border-r sm:border-r-0 sm:border-l-2 border-[#382E0E] text-white flex flex-col animate-in slide-in-from-right duration-300">
          ...
          <div className="p-5 border-b border-[#382E0E] flex items-center justify-between bg-[#120E04]">
            <div className="flex items-center gap-3">
              ...
              <div>
                <span className="font-bold text-sm sm:text-base text-white block">استوديو تخصيص الهوية والثيم</span>
                <span className="text-[11px] text-[#9CA3AF] font-medium block">معاينة حية وفورية لكافة ألوان المتجر</span>
              </div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-6">
                    ...
                    >
                      <div className="flex items-center gap-2.5">
                        ...
                        <div className="overflow-hidden">
                          <p className="font-black text-xs text-white truncate">{preset.nameAr}</p>
                          <p className="text-[10px] text-[#9CA3AF] font-mono mt-0.5">{preset.nameEn}</p>
                          <div className="flex items-center gap-1 mt-1">
                            <span className="w-2.5 h-2.5 rounded-full border border-white/20" style={{ backgroundColor: preset.headerBg }} title="الهيدر" />
                            <span className="w-2.5 h-2.5 rounded-full border border-white/20" style={{ backgroundColor: preset.pageBg }} title="الخلفية" />
                            <span className="w-2.5 h-2.5 rounded-full border border-white/20" style={{ backgroundColor: preset.primary }} title="اللون الأساسي" />
                          </div>
                        </div>
                      </div>
            ...
            <div className="space-y-3 p-4 rounded-2xl bg-[#1C1707] border border-[#382E0E]">
              ...
              <div className="p-3 rounded-2xl bg-[#120E04] border border-[#382E0E] flex items-center gap-3">
                ...
                <div className="flex-1 space-y-2">
                  ...
                  >
                    ...
                    <span>{isUploading ? 'جاري الرفع...' : 'رفع صورة الشعار'}</span>
                  </button>
              ...
              <div className="space-y-2 pt-1">
                <div>
                  <label className="text-[11px] text-[#9CA3AF] font-bold block mb-1">اسم المتجر</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="text-[11px] text-[#9CA3AF] font-bold block mb-1">الشعار اللفظي (Slogan)</label>
                  <input
                    type="text"
            ...
            <div className="space-y-3 p-4 rounded-2xl bg-[#1C1707] border border-[#382E0E]">
              <span className="font-bold text-xs text-white block">تخصيص درجات الألوان بدقة:</span>
              
              <div className="grid grid-cols-2 gap-3 pt-1">
                {/* Header Background */}
                <div>
                  <label className="text-[11px] text-[#9CA3AF] font-bold block mb-1">لون الهيدر (Header)</label>
                  <div className="flex items-center gap-2 bg-[#120E04] p-2 rounded-xl border border-[#382E0E]">
                    ...
                    />
                    <span className="text-xs font-mono font-bold text-white">{settings.headerBgColor || '#161205'}</span>
                  </div>
                </div>

                {/* Page Background */}
                <div>
                  <label className="text-[11px] text-[#9CA3AF] font-bold block mb-1">خلفية الصفحة (Page BG)</label>
                  <div className="flex items-center gap-2 bg-[#120E04] p-2 rounded-xl border border-[#382E0E]">
                    <input
                      type="color"
                      ...
                      className="w-7 h-7 rounded-lg cursor-pointer bg-transparent border-0"
                    />
                    <span className="text-xs font-mono font-bold text-white">{settings.pageBgColor || '#120E04'}</span>
                  </div>
                </div>

                {/* Card Background */}
                <div>
                  <label className="text-[11px] text-[#9CA3AF] font-bold block mb-1">خلفية الكروت (Cards)</label>
                  <div className="flex items-center gap-2 bg-[#120E04] p-2 rounded-xl border border-[#382E0E]">
                    <input
                      type="color"
                      ...
                      className="w-7 h-7 rounded-lg cursor-pointer bg-transparent border-0"
                    />
                    <span className="text-xs font-mono font-bold text-white">{settings.cardBgColor || '#1C1707'}</span>
                  </div>
                </div>

                {/* Footer Background */}
                <div>
                  <label className="text-[11px] text-[#9CA3AF] font-bold block mb-1">خلفية الفوتر (Footer)</label>
                  <div className="flex items-center gap-2 bg-[#120E04] p-2 rounded-xl border border-[#382E0E]">
                    <input
                      type="color"
                      ...
                      className="w-7 h-7 rounded-lg cursor-pointer bg-transparent border-0"
                    />
                    <span className="text-xs font-mono font-bold text-white">{settings.footerBgColor || '#0C0903'}</span>
                  </div>
                </div>

                {/* Primary Brand Color */}
                <div>
                  <label className="text-[11px] text-[#9CA3AF] font-bold block mb-1">اللون الأساسي (Primary)</label>
                  <div className="flex items-center gap-2 bg-[#120E04] p-2 rounded-xl border border-[#382E0E]">
                    <input
                      type="color"
                      ...
                      className="w-7 h-7 rounded-lg cursor-pointer bg-transparent border-0"
                    />
                    <span className="text-xs font-mono font-bold text-white">{settings.primaryColor || '#FFE600'}</span>
                  </div>
                </div>

                {/* Accent Color */}
                <div>
                  <label className="text-[11px] text-[#9CA3AF] font-bold block mb-1">اللون الثانوي (Accent)</label>
                  <div className="flex items-center gap-2 bg-[#120E04] p-2 rounded-xl border border-[#382E0E]">
                    <input
                      type="color"
                      ...
                      className="w-7 h-7 rounded-lg cursor-pointer bg-transparent border-0"
                    />
                    <span className="text-xs font-mono font-bold text-white">{settings.accentColor || '#FFD700'}</span>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div className="flex items-center justify-between">
                ...
                <label className="relative inline-flex items-center cursor-pointer">
                  <input
                    ...
                    onChange={(e) => updateSettings({ announcementEnabled: e.target.checked })}
